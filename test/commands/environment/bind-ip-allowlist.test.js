...
test('bind -- fail', async () => {
  ...
  await expect(mockSdk.addIpAllowlistBinding).toHaveBeenCalledWith('1', '3', '2', 'author')
})
...
test('bind -- success', async () => {
  ...
  await expect(mockSdk.addIpAllowlistBinding.mock.calls.length).toEqual(1)
  await expect(mockSdk.addIpAllowlistBinding).toHaveBeenCalledWith('1', '3', '2', 'author')
  await expect(cli.action.start.mock.calls[0][0]).toEqual('binding IP allowlist 3 to environment 2 (author)')
