...
test('unbind -- fail', async () => {
  ...
  await expect(mockSdk.removeIpAllowlistBinding).toHaveBeenCalledWith('1', '3', '2', 'author')
})
...
test('unbind -- success', async () => {
  ...
  await expect(mockSdk.removeIpAllowlistBinding.mock.calls.length).toEqual(1)
  await expect(mockSdk.removeIpAllowlistBinding).toHaveBeenCalledWith('1', '3', '2', 'author')
  await expect(cli.action.start.mock.calls[0][0]).toEqual('removing IP allowlist 3 binding from environment 2 (author)')
